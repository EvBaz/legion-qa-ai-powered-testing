...
Follow `@qa-mission`. Triage classifies; the human approves the fix before you
patch.

## Prerequisites

**Require triage's drift classification and human approval.** Before any edit:
...
3. Confirm the human chose **fix test / option B** per `@qa-mission`. Do not
   patch the POM until they approve.
...
Cross-reference `pages/` per `@pom-conventions`
(`.agents/skills/pom-conventions/SKILL.md`). The patch target is always the
POM, never the spec.
...
1. **Auth first:** `npm run auth:status`; if invalid, `npm run auth:refresh`.
   Reuse `playwright/.auth/user.json` — full bootstrap steps in
   `.cursor/skills/explore-and-generate/SKILL.md`. Sign in manually only if
   session load fails.
2. `browser_navigate` to the page/state where the test failed.
3. `browser_snapshot` — source of truth. Find the control by **role** and
   **current accessible name**, not screenshot pixels.
4. If the element is hidden, interact only enough to reveal it (open modal,
   select row, scroll), then snapshot again.
5. Derive the **new locator** using the same hierarchy as `@pom-conventions`:
   `getByRole` → `getByLabel` → `getByText`. Never CSS/XPath/`getByTestId`.
...
```bash
npx playwright test <spec> -g "<test title>"
```

Prefer the single failing test. If `-g` isolation fails (shared setup/fixtures),
fall back to the full spec file — still confirm **zero** spec-file edits.
...
| Wait for human **fix test / option B** before patching | Patch before human approves |
...
- `@qa-mission` — human decision gate (triage → approve → act)
- `@ci-failure-triage` — `.agents/skills/ci-failure-triage/SKILL.md`; must run
  first; supplies classification
- `@pom-conventions` — `.agents/skills/pom-conventions/SKILL.md`; locator
  hierarchy and file layout
- `@jira-bug-reporter` — `.agents/skills/jira-bug-reporter/SKILL.md`; when
  triage says real app bug
- `explore-and-generate` — `.cursor/skills/explore-and-generate/SKILL.md`;
  auth bootstrap for browser MCP